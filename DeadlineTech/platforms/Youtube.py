...
BackupUrlDomain = "http://64.227.79.220:8080"
            ...
            try:
                async with session.get(song_url) as response:
                    ...
                    if status == "done":
                        ...
                        print(f"[Attempt {attempt + 1}/7] API status: Downloading... retrying")

                    elif status == "failed":
                        if RETRY_API_MODE and attempt < MAX_API_FAILED_RETRY:
                            print(f"[Attempt {attempt + 1}/3] API status: failed")
