...
# ✅ Safe start wrapper for reconnecting clients
async def safe_start(client, name):
    while True:
        try:
            ...
            LOGGER("DeadlineTech").error(f"❌ Failed to start {name}: {e}")
            traceback.print_exc()
            LOGGER("DeadlineTech").info(f"🔁 Retrying {name} in 5 seconds...")
...
async def init():
    ...
    await safe_start(userbot, "Assistant")
    await safe_start(Anony, "PyTgCalls")
